...
class Optk(SelectionMethod):
    ...
    def setup_teacher_model(self, config, logger):
        """"Retrieve the teacher model for computing irreducible loss.
        Args:
            config (dict): Configuration dictionary containing model parameters.
            logger (logging.Logger): Logger instance for logging information.
        """
        teacher_model_path = config['rholoss']['teacher_model_path']
        model_type = config['rholoss']['networks']['type']
        model_args = config['rholoss']['networks']['params']

        # Create model
        self.teacher_model = getattr(models, model_type)(**model_args)
        self.teacher_model.to(self.device)

        if teacher_model_path == 'None':
            self.train_teacher_model(config, logger)
        else:
            try:
                ...
                raise ValueError("Invalid teacher_model configuration.")

    def train_teacher_model(self, config, logger):
        """
        Train the teacher model for estimating irreducible loss.

        Args:
            config (dict): Configuration settings for training.
            logger (Logger): Logging utility.
            teacher_dataloader (DataLoader): DataLoader for teacher set.
            teacher_epochs (int): Number of epochs to train the teacher model.
        """
        
        optimizer = create_optimizer(self.teacher_model, config)
        criterion = create_teacher_criterion(config, logger)

        epochs = config['rholoss']['teacher_num_epochs']
        total_batch = len(self.teacher_dataloader_augmented)

        logger.info(f"[teacher Model] Starting training for {epochs} epochs on {total_batch} batches per epoch.")

        best_loss = float('inf')
        best_model_path = os.path.join(self.config['save_dir'], f'best_teacher.pth.tar')

        self.teacher_model.train()
        for epoch in range(epochs):
            epoch_loss = 0.0
            for i, datas in enumerate(self.teacher_dataloader_augmented):
                inputs = datas['input'].to(self.device)
                targets = datas['target'].to(self.device)
                ...
                if (i + 1) % 50 == 0 or (i + 1) == total_batch:
                    logger.info(f"[teacher Model][Epoch {epoch+1}/{epochs}] "
            ...
            logger.info(f"[teacher Model][Epoch {epoch+1}/{epochs}] Validation Loss: {avg_val_loss:.4f}")

            # Save best model if average validation loss improves
            if avg_val_loss < best_loss:
                best_loss = avg_val_loss
                torch.save(self.teacher_model.state_dict(), best_model_path)
                logger.info(f"Best teacher model updated at epoch {epoch+1}")
        
        # Save model state with least validation loss
        best_model_state = torch.load(best_model_path, map_location=self.device)

        self.teacher_model.load_state_dict(best_model_state)
    ...
    def optk_loss_selection(self, inputs, targets, indexes, selected_num_samples):
        ...
        # Get irreducible loss from teacher model
        irreducible_loss = self.original_train_dset.irreducible_loss_cache[indexes].to(total_loss.device)

        # Compute optk loss
        optk_loss = total_loss - 2 * torch.sqrt(total_loss * irreducible_loss)

        # Select samples with highest reducible loss
        _, indices = torch.topk(optk_loss, selected_num_samples, largest=True, sorted=False)
    ...
    def before_batch(self, i, inputs, targets, indexes):
        ...
        indices = self.optk_loss_selection(inputs, targets, indexes, selected_num_samples)
